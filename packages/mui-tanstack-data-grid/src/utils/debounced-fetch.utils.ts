...
export function useDebouncedFetch<T extends Record<string, any>>(
    onFetchData:
        | ((filters: Partial<TableFilters>, meta?: DataFetchMeta) => Promise<{ data: T[]; total: number }>)
        | undefined,
    ...
    const debouncedFetch = useCallback(
        async (filters: Partial<TableFilters>, optionsOrDelay: number | DebouncedFetchOptions = DEFAULT_DEBOUNCE_DELAY) => {
            ...
            return new Promise<{ data: T[]; total: number } | null>((resolve, reject) => {
                ...
                debounceTimer.current = setTimeout(async () => {
                    ...
                    try {
                        ...
                        if (requestId === latestRequestIdRef.current) resolve(result);
                        else resolve(null);
                    } catch (error) {
                        if (requestId === latestRequestIdRef.current) reject(error);
                        else resolve(null);
                    } finally {
