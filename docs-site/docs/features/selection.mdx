...
The bulk-actions bar defaults to the primary palette; **retint it** with the `--dt-bulkbar-bg` /
`--dt-bulkbar-fg` tokens, `styleOverrides.bulkActionsToolbar`, or `slotProps.bulkActionsToolbar` — see
[Toolbar → Bulk-actions bar](/features/toolbar#bulk-actions-bar).
