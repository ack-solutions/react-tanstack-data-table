...
You can also start/stop a row edit from [`apiRef`](/api/api-ref): `apiRef.current.editing.startRowEdit(rowId)`
...
:::note
Row edit mode is client-data and top-level rows only (not under virtualization or in tree sub-rows). A page
change cancels the in-flight edit.
:::
