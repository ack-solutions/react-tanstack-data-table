...
`stateKey` and `defaultHiddenColumns` were never `<DataTable>` props — they belonged to the example
app's `DataGrid` wrapper, which persisted state and converted hidden-column names into
`initialState.columnVisibility`. The grid itself has no built-in persistence; use `initialState`
(uncontrolled) or the controlled `onColumnVisibilityChange` / `onDataStateChange` callbacks plus your
own storage. See the [example app](https://github.com/ack-solutions/react-tanstack-data-table) for a
ready-made wrapper.
:::
